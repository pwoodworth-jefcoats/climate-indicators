...
#| include: false
### Download data
# The following .csv files will be downloaded from <https://gml.noaa.gov/ccgg/trends/data.html>:
...
# Set the URLs. (these may change year to year so make sure to double check first)
# Monthly CO2 Concentration
url_atm_co2 <- "https://gml.noaa.gov/webdata/ccgg/trends/co2/co2_mm_mlo.csv"
# Annual Growth Rates
url_ann_gr  <- "https://gml.noaa.gov/webdata/ccgg/trends/co2/co2_gr_mlo.csv"
# Annual concentrations, so that we're not providing a potentially different value
url_ann_co2  <- "https://gml.noaa.gov/webdata/ccgg/trends/co2/co2_annmean_mlo.csv"

# Download as tibbles
atm_co2 <- read_csv(url_atm_co2, comment = '#')
ann_gr <- read_csv(url_ann_gr, comment = '#')
ann_co2 <- read_csv(url_ann_co2, comment = '#')

# Save for a complete record if you want. Appending report year to the file name for record keeping. 
# write.csv(atm_co2, file=paste0(Dir, '/co2_mm_mlo_', RptYr, '.csv'))
# write.csv(ann_gr, file=paste0(Dir, '/co2_gr_mlo_', RptYr, '.csv'))
# write.csv(annotation_custom(), file=paste0(Dir, '/co2_annmean_mlo_', RptYr, '.csv'))
```

```{r}
#| include: false

### Uncomment below if you have already downloaded the data and want to read it in instead. 
...
# This section will print out an error check if there is something so that will need to be commented out prior to final rendering (or removed in Word after rendering)
