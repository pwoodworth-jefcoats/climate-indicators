...
x_grid <- c(min(indicator_data$Date_Time), 
            seq.POSIXt(dmy_hm(010119600000), max(indicator_data$Date_Time), by='10 year'),
            max(indicator_data$Date_Time))
# x axis limit
ts_x_lim <- c(min(indicator_data$Date_Time), max(indicator_data$Date_Time)+(86400*16))
...
# pdf(paste(Dir, '/AtmosphericCO2_ts_', RptYr, '.pdf', sep = ""), width=9, height=6)
# par(mar=c(8.6, 5.1, 2.1, 2.1))
# plot(indicator_data$Date_Time, indicator_data$Value, type = "l", lwd = 2, 
#      xlim = ts_x_lim, ylim = c(300, 435), 
...
#      col = pal[4],
#      xlab = " ", ylab = " ")
# axis(1, at = x_grid, label = c(" ", seq(1960, 2025, 10), 2025), col = "transparent", col.ticks = "transparent",xpd=T) 
# mtext(adj = 0, side=1, line=0.9, text=1960, xpd=T) # This is to add in the 1960 that is too close to the edge for the plot to print for some reason
# axis(2, at = c(seq(300, 420, 20), 435), las = 1, col = "transparent", col.ticks = "transparent")
# grid.roundrect(x=0.51, y=0.08, width=0.6, height=0.1, 
#                r=unit(0.3, "snpc"), gp=gpar(fill="gray90", col='gray90'), just='centre')
# legend('bottom', 
#        legend=c(paste0('Monthly average, 1958-',RptYr-1), paste0('Annual average, 1958-',RptYr-1), paste0('Monthly average, ',RptYr), paste0('Annual average, ',RptYr)), 
#        col=c(pal[1], pal[3], pal[2], pal[4]), lty=1, 
#        lwd=1.75, 
#        bg='transparent', 
#        horiz=F, ncol=2, 
#        inset = -0.4, xpd=T, 
