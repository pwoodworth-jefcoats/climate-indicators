...
Pos_idx <- which(indicator_data$Value > 0)
Neg_idx <- which(indicator_data$Value < 0)
...
pdf(paste(Dir, '/PDO_ts_', RptYr, '.pdf', sep = ""), width=8, height=5)
par(mar=c(4.1, 4.1, 2.1, 2.1))
plot(c(min(indicator_data$Date_Time), max(indicator_data$Date_Time)), c(0, 0), col = "black", 
     type = "l", lwd = 1,
     xlim = ts_x_lim, ylim = c(-5, 5),
     xaxt = "n", yaxt = "n", # no axes ticks, so we can customize them
     xaxs = "i", yaxs = "i", # remove the padding R adds
     bty = "n", # remove box
     xlab = "Year", ylab = "Pacific Decadal Oscillation (PDO)",
     # put a grid behind the plot
     panel.first = c(abline(v = x_grid, col = "#F1F3F3"),
                     abline(h = c(seq(-5, 5, 1)), col = "#F1F3F3")),
     font.lab = 2)
par(new = TRUE)
stem(indicator_data$Date_Time[Pos_idx], indicator_data$Value[Pos_idx], pal[2])
stem(indicator_data$Date_Time[Neg_idx], indicator_data$Value[Neg_idx], oceans[2])
axis(1, at = x_grid, label = c("1854", " ", seq(1880, 2020, 20), " "), col = "transparent", col.ticks = "transparent") 
axis(2, at = c(seq(-5, 5, 1)), las = 1, col = "transparent", col.ticks = "transparent")
legend('topright',
       legend=c('Negative PDO', 'Positive PDO'),
       col=c(oceans[2], pal[2]),
       pch=c(15,15),
       pt.cex = 2,
       bty='n', 
       horiz = F,
...
\
**Sourced from:** NOAA (2025b), Mantua (1997), and Newman (2016). Graphics produced in part using Stawitz and Li (2025).  
\
