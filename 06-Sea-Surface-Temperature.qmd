...
```{r}
#| include: FALSE
# To plot the maps, you'll need to run map_data_for_dashboard.R that's in the PreProcessing folder of the Indicator_Dashboard folder in this repository. 
```
...
SST data are geographically subset and spatially averaged using the pyFerret script `SSTaccess.jnl` which can be found in the Sea_Surface_Temperature folder.  You could do this step in R, but I use [PyFerret](https://ferret.pmel.noaa.gov/Ferret/) because it's freely available software developed by NOAA specifically for working with large gridded datasets.  
\
