...
ridgefunc <- function(dealerDat, col, L50, L50col, xmax, xmin=0, cutoff=0.01, overlap=2, alpha=0.8, bwidth=2) {
  ...
  rdg <- ggplot(dealerDat, aes(x=WholeWeight, y=Year, group=Year)) + 
    ...
    scale_y_reverse(limits=c(RptYr+0.2, 1998), expand=c(0,0), breaks=seq(RptYr, 2000,by=-1)) + 
...
i <- ridgefunc(dealer, col='#63a298', L50=-10, L50col=NA, xmax=90, xmin=-5, cutoff=0.02)
#a2 <- violfunc(dealer, '#63a298', w, -1, NA, yMax=200)
j <- ridgefunc(dealerBE, '#5b84a1', BET_L50_kg, L50col='gray30', xmax=105, bwidth=2)
k <- ridgefunc(dealerSW, '#ffba37', c(SW_L50_kg_f, SW_L50_kg_m), 'gray30', xmax=310, xmin=-10, overlap=2, alpha=1, bwidth=5)

ggsave(plot = i, filename = paste0(Dir, '/SAFEdealerRidgeAllFish_', RptYr, '.png'), width = 8, height = 16, dpi=300, units='in')
ggsave(plot = j, filename = paste0(Dir, '/SAFEdealerRidgeBET_', RptYr, '.png'), width = 8, height = 16, dpi=300, units='in')
ggsave(plot = k, filename = paste0(Dir, '/SAFEdealerRidgesSWO_', RptYr, '.png'), width = 8, height = 16, dpi=300, units='in')
```
...
png(paste0(Dir, '/SAFEdealerWeightDensDist_', RptYr, '.png'), width=5, height=7, res=300, units='in')
par(mfrow=c(3,1))
