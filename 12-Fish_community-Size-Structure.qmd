...
ridgefunc <- function(dealerDat, col, L50, L50col, xmax, xmin=0, cutoff=0.01) {
  # Get the total number of fish sold each year
  numSold <- aggregate(NUM_SOLD ~ Year, data=dealerDat, FUN=sum)
  numSold$pretty <- prettyNum(c(numSold$NUM_SOLD), big.mark=",")
  ...
  rdg <- ggplot(dealerDat, aes(x=WholeWeight, y=Year, group=Year)) + 
    ...
    scale_y_reverse(limits=c(RptYr+0.5, 1998), expand=c(0,0), breaks=seq(RptYr, 2000,by=-4)) + 
...
#a2 <- violfunc(dealer, '#63a298', w, -1, NA, yMax=200)
j <- ridgefunc(dealerBE, '#5b84a1', BET_L50_kg, 'gray30', 105)
k <- ridgefunc(dealerSW, '#ffba37', c(SW_L50_kg_f, SW_L50_kg_m), 'gray30', 310, -10)
