...
dealerRaw <- readRDS(paste0(Dir, '/SAFE_dealer_', RptYr, '_processed.rds'))
logbookRaw <- readRDS(paste0(Dir, '/SAFE_logbook_', RptYr, '.rds'))
...
dealer <- dealerRaw %>%
  ...
  select(TRIP_NUM, REPORT_DATE, NUM_SOLD, WholeWeight, Year, Quarter) %>%
  mutate(WholeWeight=ifelse(WholeWeight > 100, 100, WholeWeight))
...
for (q in 1:4) {
  cCPUE[[q]] <- dealer %>% 
    ...
    select(WholeWeight) %>% 
    lapply(hist, plot=F)
  cCPUE[[q]]$WholeWeight$CPUE <- cCPUE[[q]]$WholeWeight$counts/efrtTot$EffortClim[q]
}
```
...
cXpos <- cCPUE[[1]]$WholeWeight$mids
...
# Start plotting
# Save file
png(paste0(Dir, '/SAFE_SizeFreqHist_', RptYr, '.png'), height=8, width=5.5, unit='in', res=300)
# Set plot dimensions
...
for (y in yrRange) {
  ...
  for (q in qIdx$Quarter) {
    ...
    qWeight <- dealer %>% filter(Quarter==q, Year==y) %>% select(WholeWeight) %>% lapply(hist, plot=F)
    qCPUE <- qWeight$WholeWeight$counts/qEffort$Effort
    qCPUE[is.infinite(qCPUE)] <- rep(0, 20)
    
    par(mar=c(0.25,3,0,1))
    plot(cCPUE[[q]]$WholeWeight$mids, cCPUE[[q]]$WholeWeight$CPUE, type='l', lwd=2, col='gray50', xlab=NA, ylab=NA, axes=F, xaxs='i', yaxs='i',
         xlim=c(0,100), ylim=c(0,1.2))
    par(new=T)
    plot(qWeight$WholeWeight$mids, qCPUE, type='l', lwd=3, col=pal[q+2], xlab=NA, ylab=NA, axes=F, xaxs='i', yaxs='i',
         xlim=c(0,100), ylim=c(0,1.2))
    axis(2, las=1, at=c(0,0.5,1,1.2), labels=F)
...
```{r echo=FALSE, out.width="5.5in", fig.align='center'}
# Figure caption info
# fig-wpue, fig.cap=paste0('Quarterly deep-set bigeye tuna weight per unit effort for ', RptYr-3, '-', RptYr-1, ' (color) and the climatological average (2000-2009)'), out.width='70%', fig.align='center', 

knitr::include_graphics(paste0(Dir, '/SAFE_SizeFreqHist_', RptYr, '.png'), )
...
\
**Status:** There is a peak in two-year-old bigeye CPUE starting in the third quarter of `r RptYr` that persists through the fourth quarter, suggesting that there will be a peak in the CPUE of four- and five-year-old bigeye in `r RptYr+1` or `r RptYr+2`.\
\
**Description:** Quarterly time series of bigeye weight-per-unit-effort (WPUE) in hooks set is presented for the previous two years. Fish weights are those of bigeye tuna received at the Honolulu auction. The Honolulu auction reports weights for gilled and gutted fish. A conversion factor is used to calculate the whole fish weights used for this indicator (Langley et al. 2006). Note the quarterly (colored) and climatological (grey) distributions of bigeye tuna weight-per-unit-effort in Figure??. Bigeye weights are from sets using $\ge$ 15 hooks per float.\
\
**Timeframe:** Quarterly.\
\
...
\
**Sourced from:** PIFSC (2026a), PIFSC (2026b) and the Hawaiʻi Division of Aquatic Resources, and Langley et al. (2006).
