...
dealerProc <- readRDS(paste0(Dir, '/SAFE_dealer_', RptYr, '_processed.rds'))
logbook <- readRDS(paste0(Dir, '/SAFE_logbook_', RptYr, '.rds'))
```
...
write.csv(dealerYear, paste0(here('Bigeye_Forecast'), '/CPUEWPUE_', RptYr, '.csv'), quote = F, row.names = F)
```
...
recIndex <- dealerYear %>% 
  select(LANDYR, CPUEsmall) %>% 
  rename(Year=LANDYR, `Fish per 1,000 Hooks`=CPUEsmall)
# Round to two significant figures/decimal places
recIndex$`Fish per 1,000 Hooks` <- format(round(recIndex$`Fish per 1,000 Hooks`,2), nsmall=2)
# Save file
...
\
**Status:** In `r RptYr`, the CPUE of bigeye ≤ 15 kg was `r round(dealerYear$CPUEsmall[which(dealerYear$LANDYR==RptYr)],2)` fish per 1,000 hooks set. This is an increase from `r RptYr-1` and close to the mean CPUE of small bigeye tuna of `r round(mean(dealerYear$CPUEsmall, na.rm=T),2)` fish per 1,000 hooks set. The increase in CPUE for bigeye tuna ≤ 15 kg shows the beginning signs of a recruitment pulse.\
\
