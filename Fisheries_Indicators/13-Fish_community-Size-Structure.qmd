...
violfunc <- function(x, col, img, L50, L50col, dotData, imgMin=2022, yMax=max(x$WholeWeight)) {
  ...
  for (i in 1:length(numSold$Year)) {
    p <- p + annotation_custom(grob=textGrob(label=numSold$pretty[i], gp=gpar(cex=0.6, color='gray20'), rot=45), ymin=yMin, ymax=yMin, xmin=numSold$Year[i]+0.4, xmax=numSold$Year[i]+0.4)
  }
