...
dealer <- dealerProc %>% 
  filter(REPORT_DATE < as.Date(paste0(RptYr, '-01-01')),
         SPECIES_FK == 6,
         SET_TYPE == 'D') %>% 
  mutate
dealer$Num_Small <- ifelse(dealer$WholeWeight <= 15, dealer$NUM_SOLD, 0)
...
# Make plot
png(paste0('TEST_DELETE_THIS_RecIndexRaw_', RptYr, '.png'), height=6, width=11, res=300, unit='in')
# Make raw plot
par(mar=c(5,9,1,4.5))
