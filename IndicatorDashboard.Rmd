...
resource_files:
  ...
  - "CO2-Indicator-Table-2022-Monthly-Variables.png"
  - "Chl-Indicator-Table-2022-Monthly-Variables.png"
  - "ENSO-Indicator-Table-2022-Monthly-Variables.png"
  - "MD50-Indicator-Table-2022-Monthly-Variables.png"
  - "PDO-Indicator-Table-2022-Monthly-Variables.png"
  - "SST-Indicator-Table-2022-Monthly-Variables.png"
  - "TatD-Indicator-Table-2022-Monthly-Variables.png"
  - "pH-Indicator-Table-2022-Monthly-Variables.png"
...
output$ts <- renderPlotly({
  ...
  if (input$indicator == "PDO"){
      ...
      p1 <- plot_ly(indicator_data, x = dmy_hm(indicator_data$Date_Time), y = ~Value, type = "bar", height = 350,
   ...
   p1 <- p1 |> layout(title = list(text = "Indicator Time Series", x = 0.01, font = t1), 
...
output$anom_ts <- renderPlotly({
  ...
  p2 <- plot_ly(indicator_data, x = dmy_hm(indicator_data$Date_Time), y = ~Anom, height = 350,
                type = "scatter", mode = "lines", line = list(color = pal[1]),
    ...
    layout(yaxis = list(title = indicator_data$Units[1], hoverformat = '.3f'),
           title = list(text = "Anomaly Time Series", x = 0.01, font = t1),
           paper_bgcolor='transparent',
           plot_bgcolor='transparent',
           hovermode = "x unified", 
           hoverlabel = list(namelength = -1))
...
output$seasonal_cycle <- renderPlotly({
  ...
  p3_tmp <- Seasonal |>
    group_by(Month) |>
    group_map(~{plot_ly(.x, x = ~Year, y = ~Value, type = "scatter", color = ~Year, name = ~Month_Abb, height = 350,
    ...
    layout(title = list(text = "Seasonal Cycle Plot", x = 0.01, font = t1),
           paper_bgcolor='transparent',
           plot_bgcolor='transparent')
...
output$seasonal_dots <- renderPlotly({
  ...
  p3 <- plot_ly(Seasonal, x = ~Month_Abb, y = ~Value, height = 350,
                type = 'scatter', mode = 'markers', color = ~Year, text = ~ID[1],
                hovertemplate = paste('<b>%{text}</b>: %{y}',
                                      '<br><b>Year</b>: %{marker.color}',
                                      '<extra></extra>')) |>
    layout(title = list(text = "Seasonal Points Plot", x = 0.01, font = t1), 
           paper_bgcolor='transparent',
           plot_bgcolor='transparent',
...
output$time_series_table <- renderDT({
  ...
  IndicatorTimeSeries}, 
  selection = 'none',
  caption = htmltools::tags$caption(style = 'color:black;', "Indicator Time Series Data:"),
...
DTOutput("time_series_table", width = "90%")
...
output$anom_table <- renderDT({
  ...
  selection = 'none',
  height = '20%',
  caption = htmltools::tags$caption(style = 'color:black;', "Anomaly Time Series Data:"),
  options = list(searching = FALSE, paging = TRUE, info = FALSE, lengthChange = FALSE,
                 columnDefs = list(list(className = 'dt-right', targets = c(1, 2))))
...
renderValueBox({
  ...
  if (input$indicator == "pH"){
    ann_mean_RptYr <- indicator_data |> filter(Year == RptYr - 1) |>
    summarise(Value = mean(Value, na.rm = TRUE)) |> signif(3)
    tmp_caption <- paste(as.character(RptYr - 1), "Average")
  } else{
    ann_mean_RptYr <- indicator_data |> filter(Year == RptYr) |>
    summarise(Value = mean(Value, na.rm = TRUE)) |> signif(3) 
    tmp_caption <- paste(as.character(RptYr), "Average")
  }
  
  valueBox(value = ann_mean_RptYr, color = pal[1], caption = tmp_caption)
})
```
...
renderValueBox({
  ...
  if (input$indicator == "pH"){
    monthly_min_RptYr <- indicator_data |> filter(Year == RptYr - 1) |>
      summarise(Value = min(Value, na.rm = TRUE)) |> signif(3) 
    monthly_max_RptYr <- indicator_data |> filter(Year == RptYr - 1) |>
  ...
  valueBox(value = paste(monthly_min_RptYr, "\u2013", monthly_max_RptYr), color = pal[1], caption = tmp_caption)
})
```
...
renderValueBox({
  ...
  if (input$indicator == "pH"){
    ...
    monthly_min_PrevYrs <- indicator_data |> filter(Year < (RptYr - 1)) |>
      summarise(Value = min(Value, na.rm = TRUE)) |> signif(3) 
    monthly_max_PrevYrs <- indicator_data |> filter(Year < (RptYr - 1)) |>
      summarise(Value = max(Value, na.rm = TRUE)) |> signif(3) 
  } else{
    # Monthly min & max for previous years
