...
output$summary_img <- renderImage({
  ...
  # Select image
  filename = here('Indicator_Dashboard', 'Images', "Indicator-Table-2022-Monthly-Variables.png")
...
output$ts <- renderPlotly({
  ...
  # Calculate annual means - ignore for ENSO and PDO
  # as annual means are not relevant for those indicators
  if (input$indicator == "ENSO" | input$indicator == "PDO"){
      ann_vals <- NULL
  } else{
...
output$anom_ts <- renderPlotly({
  ...
  if (input$indicator %in% anom_list){
