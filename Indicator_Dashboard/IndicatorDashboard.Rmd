...
runtime: shiny
resource_files:
  - "Data/Dashboard_Data_2024.csv"
  - "Data/Dashboard_Map_Data_2024.csv"
  - "Data/rnatearth_coast.RData"
  - "Data/rnatearth_land.RData"
  - "Images/Indicator-Table-2024-Monthly-Variables.png"
  - "Images/CO2-Indicator-Table-2024-Monthly-Variables.png"
  - "Images/Chl-Indicator-Table-2024-Monthly-Variables.png"
  - "Images/ENSO-Indicator-Table-2024-Monthly-Variables.png"
  - "Images/MD50-Indicator-Table-2024-Monthly-Variables.png"
  - "Images/PDO-Indicator-Table-2024-Monthly-Variables.png"
  - "Images/SST-Indicator-Table-2024-Monthly-Variables.png"
  - "Images/TatD-Indicator-Table-2024-Monthly-Variables.png"
  - "Images/pH-Indicator-Table-2024-Monthly-Variables.png"
...
report_year <- 2024 #current report year
...
indicator_data <- read_csv(here('Data', paste0('Dashboard_Data_', report_year, '.csv')))
...
output$summary_img <- renderImage({
  ...
  # Select image
  filename = here('Images', paste0(input$indicator, "-Indicator-Table-", report_year, "-Monthly-Variables.png"))

  #Return a list containing filename and alt text
  list(src = filename,
       alt = "Summary graphic that highlights the chosen variable and describes and summarizes all variables",
       width = "100%")
...
output$ts <- renderPlotly({
  ...
  } else if (input$indicator == "NPGO"){ # No annual mean or linear trend for NPGO
...
output$ts_desc <- renderUI( 
  ...
  if (input$indicator == "CO2") {
    markdown("Monthly and annual atmospheric carbon dioxide (CO2)")
  } else if (input$indicator == "SST") {
    markdown("Monthly and annual sea surface temperature (SST), as well as the long-term trend")
  } else if (input$indicator == "Chl") {
    markdown("Monthly and annual ocean color, measured as chlorophyll (chl) concentration")    
  } else if (input$indicator == "MD50") {
    markdown("Monthly and annual median phytoplankton size (MD50), measured as equvialent spherical diameter (ESD) in micrometers (um), as well as the long-term trend")    
  } else if (input$indicator == "ENSO") {
    markdown("Time series of the Oceanic Niño Index (ONI), with El Niño periods in red, La Niña periods in blue, and neutral periods in grey")
  } else if (input$indicator == "PDO") {
    markdown("Time series of the Pacific Decadal Oscillation (PDO), with positive phases in red and negative phases in blue")
  } else if (input$indicator == "pH") {
    markdown("Monthly and annual pH, as well as the long term trend")
  } else if (input$indicator == "TatD") {
    markdown("Monthly and annual temperature at 200 - 300 m depth (TatD), as well as the long-term trend")
  } else if (input$indicator == "NPGO") {
    markdown("Time series of the North Pacific Gyre Oscillation (NPGO)")
  }
)
...
output$anom_desc <- renderUI(
  ...
  if (input$indicator == "CO2") {
    markdown("Monthly and annual CO2 with the seasonal cycle removed")
  } else if (input$indicator == "SST") {
    markdown("Monthly and annual SST with the seasonal cycle removed, as well as the long-term trend in this anomaly time series")
  } else if (input$indicator == "Chl") {
    markdown("Monthly and annual chl with the seasonal cycle removed, as well as the long-term trend in this anomaly time series")    
  } else if (input$indicator == "MD50") {
    markdown("Monthly and annual MD50 with the seasonal cycle removed, as well as the long-term trend in this anomaly series")    
  }
)
...
output$seasonal_cycle <- renderPlotly({
  ...
  p3 <- Seasonal |>
    ...
    group_map(~{plot_ly(.x, x = ~Year, y = ~Value, type = "scatter", color = ~Value, name = ~Month_Abb, height = plot_height,
         ...
         colorbar(title = indicator_data$Units[1], len = 0.7, x = 1, y = 0.8, 
                  limits = c(min(Seasonal$Value, na.rm = TRUE), max(Seasonal$Value, na.rm = TRUE)))}) |>
...
output$season_desc <- renderUI(
  ...
  if (input$indicator == "CO2") {
    markdown("The seasonal cycle plot shows atmospheric carbon dioxide over time by month.  The horizontal black lines are the time series average for each month.  This figure clearly shows that atmospheric carbon dioxide is increasing over time in all months of the year.")
  } else if (input$indicator == "pH") {
    markdown("The seasonal cycle plot shows oceanic pH over time by month.  The horizontal black lines are the time series average for each month.  This figure clearly shows that oceanic pH is decreasing over time in all months of the year.")
  } else if (input$indicator == "ENSO") {
    markdown("The seasonal cycle plot shows Oceanic Niño Index (ONI) values over time by month.  The horizontal black lines are the time series average for each month.  This figure shows that there is no long-term trend in ONI values and that values tend to be closes to zero in May.")
  } else if (input$indicator == "PDO") {
    markdown("The seasonal cycle plot shows PDO values over time by month.  The horizontal black lines are the time series average for each month.  This figure shows that there is no long-term trend in PDO values.")
  } else if (input$indicator == "SST") {
    markdown("The seasonal cycle plot shows SST over time by month.  The horizontal black lines are the time series average for each month.  This figure shows both the annual SST cycle (coolest in March, warmest in September) and that SST is increasing over time in all months of the year.")
  } else if (input$indicator == "TatD") {
    markdown("The seasonal cycle plot shows temperature at depth over time by month.  The horizontal black lines are the time series average for each month.  This figure shows that there is little seasonal cycle at this depth and no clear trend over time in any month of the year.")    
  } else if (input$indicator == "Chl") {
    markdown("The seasonal cycle plot shows ocean color over time by month.  The horizontal black lines are the time series average for each month.  This figure shows the annual ocean color cycle (greatest in February, least in August).")     
  } else if (input$indicator == "MD50") {
    markdown("The seasonal cycle plot shows median phytoplankton size over time by month.  The horizontal black lines are the time series average for each month.  This figure shows the annual cycle for phytoplankton size (largest in February, smallest in September) and illustrates the small decline in median phytoplankton size across months. ")    
  } else if (input$indicator == "NPGO") {
    markdown("The seasonal cycle plot shows NPGO values over time by month.  The horizontal black lines are the time series average for each month.  This figure shows that there is no long-term trend in NPGO values.")
  }
)
...
output$map <- renderPlotly({
  ...
  if (input$indicator %in% include_anom_map_list){
    ...
    p <- ggplot() + 
      geom_raster(data = raster_df, mapping = aes(x=x_disp, y=y, fill=layer, text = labels)) + #data as raster
      ...
      scale_x_continuous(breaks=seq(120,260,30), 
                         labels = paste0(c(seq(120,180,30), seq(150,100,-30)),
                                         c(rep('°E',2),"",rep('°W',2))), expand = c(0,0)) + # adding degrees to the breaks
      scale_y_continuous(breaks=seq(0,55,15), 
                         labels = paste0(seq(0,55,15), c('',rep('°N',3))), expand = c(0,0)) + # adding degrees to the breaks
    ...
    g <- ggplotly(p, tooltip = c("labels"), height = plot_height) |>
      ...
      style(hoverinfo = "skip", traces = c(2, 3)) #don't show hoverinfo for longline fishing grounds box/label
    
    #return plot
    g
    
  } else{ #indicators without spatial data (CO2, pH, ENSO, PDO, NPGO)
    
    # ext <- st_bbox(land_proj)
    
    #make base map
    p <- ggplot() + 
      # geom_rect(aes(xmin = 120, xmax = 260, ymin = -5, ymax = 55), #rectangle for coloring ocean
      ...
      annotate("text", x = 210, y = 47, label = "Longline fishing grounds", size = 3.2, color = "black") +
    ...
    if (input$indicator == "CO2"){
      ...
      p <- p +       
        ...
        scale_y_continuous(breaks=seq(0,55,15), 
                           labels = paste0(seq(0,55,15), c('',rep('°N',3))), expand = c(0,0)) + # adding degrees to the breaks
        annotate(geom = "point", x = 204.4, y = 19.5, color = crustacean[2]) +
        annotate(geom = "text", x = 214.5, y = 19.5, color = "black", size = 3.2,
                 label = "Mauna Loa\nObservatory")
      ...
      p <- p + 
        ...
        scale_y_continuous(breaks=seq(0,55,15), 
                           labels = paste0(seq(0,55,15), c('',rep('°N',3))), expand = c(0,0)) + # adding degrees to the breaks
        annotate(geom = "point", x = 202.0, y = 22.75, color = crustacean[2]) +
        annotate(geom = "text", x = 202.0, y = 26.5, color = "black", size = 3.2,
                 label = "Station\nALOHA")
      ...
      #uses different base map to show whole -5 to 5 deg region
      p <- p + 
        geom_rect(aes(xmin = 190, xmax = 240, ymin = -5, ymax = 5),
                         color = crustacean[3], fill = crustacean[2],
                         alpha = 0.5, linewidth = 0.5) +
        # geom_sf(data = land_proj_enso, fill = "#A5A5A5", color = "#A5A5A5", linewidth = 0.5) +
        # geom_sf(data = coast_proj_enso, color = "black", linewidth = 0.5) +
        # coord_sf(xlim = c(ext$xmin, ext$xmax), ylim = c(-5, ext$ymax), expand = F) +
        borders('world2', fill='#A5A5A5', colour='gray40', linewidth=0.2) +
        coord_quickmap(expand = F, xlim=c(120,260), ylim=c(-15,55)) + #don't expand past x/y limits
        scale_x_continuous(breaks=seq(120,260,30), 
                           labels = paste0(c(seq(120,180,30), seq(150,100,-30)),
                                           c(rep('°E',2),"",rep('°W',2))), expand = c(0,0)) + # adding degrees to the breaks
        scale_y_continuous(breaks=seq(-15,55,15), 
                           labels = paste0(seq(-15,55,15), c('',rep('°N',3))), expand = c(0,0)) + # adding degrees to the breaks
        annotate("text", x = 220, y = 7, label = "Niño 3.4 Region", size = 3.2, color = "black")
      ...
      #customize tooltip
      g$x$data[[7]]$text <- "120.0W to 170.0W<br />5.0S to 5.0N<br />Niño 3.4 Region"
      ...
      p <- p + 
        geom_rect(aes(xmin = 120, xmax = 260, ymin = 20, ymax = 55),
                         color = crustacean[3], fill = crustacean[2],
                         alpha = 0.5, linewidth = 0.5) +
        # geom_sf(data = land_proj, fill = "#A5A5A5", color = "#A5A5A5", linewidth = 0.5) +
        # geom_sf(data = coast_proj, color = "black", linewidth = 0.5) +
      ...
      # Show NPGO calculation region
      p <- p + 
        geom_rect(aes(xmin = 180, xmax = 250, ymin = 25, ymax = 62),
                         color = crustacean[3], fill = crustacean[2],
                         alpha = 0.5, linewidth = 0.5) +
        # geom_sf(data = land_proj, fill = "#A5A5A5", color = "#A5A5A5", linewidth = 0.5) +
        # geom_sf(data = coast_proj, color = "black", linewidth = 0.5) +
        # coord_sf(xlim = c(ext$xmin, ext$xmax), ylim = c(ext$ymin, ext$ymax), expand = F) +
        borders('world2', fill='#A5A5A5', colour='gray40', linewidth=0.2) +
        coord_quickmap(expand = F, xlim=c(120,260), ylim=c(0,65)) + #don't expand past x/y limits
        scale_x_continuous(breaks=seq(120,260,30), 
                           labels = paste0(c(seq(120,180,30), seq(150,100,-30)),
                                           c(rep('°E',2),"",rep('°W',2))), expand = c(0,0)) + # adding degrees to the breaks
        scale_y_continuous(breaks=seq(0,65,15), 
                           labels = paste0(seq(0,65,15), c('',rep('°N',3))), expand = c(0,0)) + # adding degrees to the breaks
        annotate("text", x = 220, y = 27, 
                 label = "NPGO Region", size = 3.2, color = "black")
      ...
      #customize tooltip
      g$x$data[[7]]$text <- "180.0 to 110.0W<br />25.0N to 62.0N<br />NPGO Region"
...
output$map_desc <- renderUI( 
  ...
  if (input$indicator == "CO2") {
    markdown("Atmospheric CO2 data are gathered at the Mauna Loa Observatory on the island of Hawaiʻi.  The longline fishing grounds are also shown for reference. ")
  } else if (input$indicator == "pH") {
    markdown("Oceanic pH data are gathered at Station ALOHA, north of the island of Oahu.  The longline fishing grounds are also shown for reference.")
  } else if (input$indicator == "ENSO") {
    markdown("The Oceanic Niño Index is measured across the Niño 3.4 Region.  The longline fishing grounds are also shown for reference.")
  } else if (input$indicator == "PDO") {
    markdown("The Pacific Decadal Index is measured across the North Pacific basin, north of 20°N.  The longline fishing grounds are also shown for reference.")
  } else if (input$indicator == "SST") {
    markdown("This map shows the climatological average SST (1985-2009).  The data in the graphs above is averaged over the longline fishing grounds.")
  } else if (input$indicator == "TatD") {
    markdown("This map shows the climatological average temperature at depth (1980-2009).  The data in the graphs above is averaged over the longline fishing grounds.")    
  } else if (input$indicator == "Chl") {
    markdown("This map shows the climatological average ocean color (1998-2009).  The data in the graphs above is averaged over the longline fishing grounds.")    
  } else if (input$indicator == "MD50") {
    markdown("This map shows the climatological average median phytoplankton size (1998-2009).  The data in the graphs above is averaged over the longline fishing grounds.")    
  } else if (input$indicator == "NPGO") {
    markdown("The North Pacific Gyre Oscillation index is measured across the region from 180° to 110°W and 25°N to 62°N.  The longline fishing grounds are also shown for reference.")
  } 
)
...
output$anom_map <- renderPlotly({
  ...
  #first build in ggplot
  p_anom <- ggplot() + 
    geom_raster(data = raster_anom_df, mapping = aes(x=x_disp, y=y, fill=layer, text = labels)) +
    geom_rect(aes(xmin = 180, xmax = 240, ymin = 15, ymax = 45), color = "black", fill = NA, linewidth = 0.5)  +
    annotate("text", x = 210, y = 47, label = "Longline fishing grounds", size = 3.2, color = "black") +
    anom_fill_scale +
    # geom_sf(data = land_proj, fill = "#A5A5A5", color = "#A5A5A5", linewidth = 0.5) +
...
output$anom_map_desc <- renderUI(
  ...
  #indicator-specific descriptions for seasonal plot
  if (input$indicator == "SST") {
    markdown("This anomaly map shows the difference between SST in the report year (2024) and the long-term average (1985-2009).  Areas that were warmer in 2024 are red and areas that were cooler are blue.")
  } else if (input$indicator == "TatD") {
    markdown("This anomaly map shows the difference between temperature at depth in the report year (2024) and the long-term average (1980-2009).  Areas that were warmer in 2024 are red and areas that were cooler are blue.")    
  } else if (input$indicator == "Chl") {
    markdown("This anomaly map shows the difference between ocean color in the report year (2024) and the long-term average (1998-2009).  Areas with greater chl concentration in 2024 are red and areas with lower chl concentrations are blue.")    
  } else if (input$indicator == "MD50") {
    markdown("This anomaly map shows the difference between median phytoplankton size in the report year (2024) and the long-term average (1998-2009).  Areas with larger phytplankotn in 2024 are red and areas with smaller phytoplankton are blue.")    
  }  
...
renderValueBox({
  ...
  if (input$indicator == "pH"){
    ...
    first_year <- indicator_data |> filter(Year < (report_year - 1)) |>
      summarise(Year = min(Year, na.rm = TRUE))
    last_year <- indicator_data |> filter(Year < (report_year - 1)) |>
      summarise(Year = max(Year, na.rm = TRUE))
    tmp_caption <- paste(first_year, "\u2013", last_year, "Long-Term Range")
  } else{
    # Monthly min & max for previous years
    #for all other indicators, use report year
    ...
    first_year <- indicator_data |> filter(Year < (report_year)) |>
      summarise(Year = min(Year, na.rm = TRUE))
    last_year <- indicator_data |> filter(Year < (report_year)) |>
      summarise(Year = max(Year, na.rm = TRUE))
    tmp_caption <- paste(first_year, "\u2013", last_year, "Long-Term Range")
  }

  #the "\u2013" here is the unicode m-dash
  valueBox(value = paste(monthly_min_PrevYrs, "\u2013", monthly_max_PrevYrs), color = pal[1], caption = tmp_caption)
})
```
