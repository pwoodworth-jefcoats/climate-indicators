...
prep_map_data <- function(indicator, raster_ann, raster_clim, min_x, max_x, min_y, max_y, target_res){
  ...
  raster_df <- raster_ann %>%
    ...
    mutate(x = ifelse(x <= 0, x + 180, -(180 - x))) %>%
    mutate(x_disp = ifelse(x <= 0, x + 180, -(180 - x))) %>%
    # mutate(x_disp = ifelse(x > 180, x-360, x)) %>%
    # rename(x=x_disp, x_disp=x_180) %>%  
...
# #get lon coordinates from cropped data#get lon coordinates from cropped data
